...
import { HiOutlineStar, HiArrowCircleRight, HiRefresh, HiLightBulb } from 'react-icons/hi';
...
export default function SuggestedPreferences({ onSelectPreference }: SuggestedPreferencesProps) {
  ...
  const fetchPreferences = async () => {
    try {
      ...
      // Add onlyAutoSave=true parameter to only show AI-generated suggestions
      const response = await fetch('/api/save-preferences?onlyAutoSave=true');
      const data = await response.json();
      
      if (data.success) {
        // Get only the 3 most recent preferences to show as "AI suggestions"
        const recentPreferences = data.preferences.slice(0, 3);
