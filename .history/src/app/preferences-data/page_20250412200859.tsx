...
export default function PreferencesDataPage() {
  ...
  // Filter options
  const [includeAutoSave, setIncludeAutoSave] = useState(false);
  const [onlyAutoSave, setOnlyAutoSave] = useState(false);
  const [format, setFormat] = useState<'ai' | 'default'>('ai');
  ...
  const fetchPreferences = async () => {
    try {
      ...
      params.set('includeAutoSave', includeAutoSave.toString());
      params.set('onlyAutoSave', onlyAutoSave.toString());
      
      const response = await fetch(`/api/preferences-export?${params.toString()}`);
  ...
  useEffect(() => {
    fetchPreferences();
  }, [includeAutoSave, onlyAutoSave, format]);
          ...
          <div className="flex flex-wrap gap-4">
            <div className="flex items-center">
              <input
                type="checkbox"
                id="includeAutoSave"
                checked={includeAutoSave}
                onChange={() => {
                  setIncludeAutoSave(!includeAutoSave);
                  if (!includeAutoSave) {
                    setOnlyAutoSave(false);
                  }
                }}
                className="mr-2 h-4 w-4 text-yellow-500 focus:ring-yellow-400 border-gray-300 rounded"
              />
              <label htmlFor="includeAutoSave" className="text-sm text-gray-700">
                Include auto-saved data
              </label>
            </div>
            ...
            <div className="flex items-center">
              <input
                type="checkbox"
                id="onlyAutoSave"
                checked={onlyAutoSave}
                onChange={() => {
                  setOnlyAutoSave(!onlyAutoSave);
                  if (!onlyAutoSave) {
                    setIncludeAutoSave(true);
                  }
                }}
                className="mr-2 h-4 w-4 text-yellow-500 focus:ring-yellow-400 border-gray-300 rounded"
              />
              <label htmlFor="onlyAutoSave" className="text-sm text-gray-700">
                Only auto-saved data
              </label>
            </div>
            ...
            <div className="flex items-center ml-auto">
              <select
                value={format}
                onChange={(e) => setFormat(e.target.value as 'ai' | 'default')}
                className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-yellow-400"
              >
                <option value="ai">AI-Friendly Format</option>
                <option value="default">Raw Data Format</option>
              </select>
            </div>
          </div>
        </div>
