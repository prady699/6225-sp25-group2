    ...
    <div className="min-h-screen bg-gray-50 py-16 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        {currentStep === 0 && showSuggestions && (
          <div className="mt-6">
            <SuggestedPreferences onSelect={handleSelectPreference} />
          </div>
        )}
        ...
        <div className="flex justify-between">
          ...
          <button
            onClick={handleNext}
            className={`px-6 py-3 rounded-xl bg-yellow-400 text-gray-900 font-medium hover:bg-yellow-500 ${
              currentStep === 0 && !formData.location
                ? 'opacity-50 cursor-not-allowed'
                : ''
            }`}
            disabled={currentStep === 0 && !formData.location}
          >
            {currentStep === steps.length - 1 ? 'Find Properties' : 'Next'}
          </button>
        </div>
      </div>
    </div>
